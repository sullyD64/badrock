...
local opt = {}
    ...
    local function onFxMuteSwitchPress( event )
        ...
        if (switch.isOn) then 
            ...
            audio.setVolume( pausePanel.bgVolume.value, { channel=2 } )
            audio.setVolume( pausePanel.bgVolume.value, { channel=3 } )
            audio.setVolume( pausePanel.bgVolume.value, { channel=4 } )
            audio.setVolume( pausePanel.bgVolume.value, { channel=5 } )
        end
    end  
    ...
    opt.panel = utility.newPanel{
        ...
        opt.panel.background = display.newImageRect("misc/panel.png",opt.panel.width, opt.panel.height-20)
